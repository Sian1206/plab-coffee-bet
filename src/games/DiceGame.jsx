...
function Dice3D({ value, rolling, color, size = 52, angle = 0 }) {
  ...
  const rx = rolling ? 0 : -20;
  const ry = rolling ? 0 : -30;

  const Face = ({ dots, transform, dimmed }) => (
    <div style={{
      position: "absolute", width: size, height: size,
      background: dimmed ? `linear-gradient(135deg,${color}77,${color}44)` : `linear-gradient(135deg,${color}ee,${color}aa)`,
      border: "1.5px solid rgba(255,255,255,0.2)", borderRadius: size * 0.13,
      display: "flex", alignItems: "center", justifyContent: "center",
      transform, backfaceVisibility: "hidden", boxShadow: "inset 0 0 10px rgba(0,0,0,0.35)",
    }}>
      <svg width={size} height={size}>
        {(dots||[]).map(([cx,cy],i) => <circle key={i} cx={cx/100*size} cy={cy/100*size} r={dotR} fill="rgba(255,255,255,0.9)" />)}
      </svg>
    </div>
  );
  ...
  return (
    <div style={{ width: size, height: size, position: "relative", perspective: size*5 }}>
      <div style={{ width: size, height: size, position: "relative", transformStyle: "preserve-3d", transform: `rotateX(${rx}deg) rotateY(${ry}deg)`, animation: rolling ? "roll3d 0.2s linear infinite" : "none", transition: rolling ? "none" : "transform 0.5s ease" }}>
...
export default function DiceGame({ participants, bet, onBack, onHome }) {
  ...
  const [penaltyMode, setPenaltyMode] = useState("high"); // high=최고점이 커피, low=최저점이 커피
  const [phase, setPhase] = useState("setup");
  const [roundParticipants, setRoundParticipants] = useState([]); // 현재 라운드 참가자
  ...
  const startGame = () => {
    setRoundParticipants([...allParticipants]);
    setCurrentIdx(0);
    setRoundResults([]);
    setPhase("playing");
  };
  ...
  const endDrag = (cx, cy) => {
    ...
    const baseVx = (dx/dt)*0.04, baseVy = (dy/dt)*0.04;
  ...
  const doRoll = (speed=80) => {
    ...
    rollRef.current = setInterval(() => {
      ...
      if (tick >= ticks) {
        ...
        setRolling(false);
        setTimeout(() => {
          const name = roundParticipants[currentIdx];
          const color = COLORS[allParticipants.indexOf(name) % COLORS.length];
          const newResults = [...roundResults, { name, d1, d2, total: d1+d2, color }];
          ...
          if (currentIdx+1 >= roundParticipants.length) {
            // 라운드 종료 — 동점자 확인
            const scores = newResults;
            const penaltyScore = penaltyMode === "high"
              ? Math.max(...scores.map(r=>r.total))
              : Math.min(...scores.map(r=>r.total));
            const penalized = scores.filter(r => r.total === penaltyScore);
            if (penalized.length === 1) {
              setPhase("done");
            } else {
              // 동점자 재경기
              setRoundParticipants(penalized.map(r=>r.name));
              setCurrentIdx(0);
              setRoundResults([]);
              setPhase("rematch");
            }
  ...
  useEffect(() => {
    ...
    const loop = () => {
      const upd = (d) => {
        d.vx *= 0.89; d.vy *= 0.89; d.x += d.vx; d.y += d.vy; d.angle += d.vx*1.5;
      ...
      dice1Ref.current = upd(dice1Ref.current);
      dice2Ref.current = upd(dice2Ref.current);
      setDicePos({ d1:{...dice1Ref.current}, d2:{...dice2Ref.current} });
  ...
  const reset = () => {
    setPhase("setup"); setCurrentIdx(0); setRoundResults([]); setRoundParticipants([]);
  ...
  const penaltyScore = roundResults.length > 0
    ? (penaltyMode === "high" ? Math.max(...roundResults.map(r=>r.total)) : Math.min(...roundResults.map(r=>r.total)))
    : 0;
  const currentColor = roundParticipants[currentIdx] ? COLORS[allParticipants.indexOf(roundParticipants[currentIdx]) % COLORS.length] : "#eab308";
  ...
  return (
    <GameLayout bet={bet} onBack={onBack} onHome={onHome}>
      <div style={{ display:"flex", flexDirection:"column", alignItems:"center", gap:13 }}>
        ...
        {phase === "setup" && (
          <>
            <div style={{ fontSize:13, color:"#aaa", textAlign:"center", lineHeight:1.8 }}>
              드래그 방향으로 주사위 2개를 던져요!<br/>합산 점수로 커피 내는 사람을 가립니다
            </div>
            <div style={{ background:"rgba(255,255,255,0.04)", border:"1px solid rgba(255,255,255,0.1)", borderRadius:18, padding:"16px", width:"100%" }}>
              ...
              <div style={{ display:"flex", gap:10 }}>
                <button onClick={() => setPenaltyMode("high")} style={{ flex:1, background: penaltyMode==="high" ? "linear-gradient(135deg,#ef4444,#dc2626)" : "rgba(255,255,255,0.07)", border:"none", borderRadius:12, padding:"12px", color: penaltyMode==="high"?"#fff":"#aaa", fontSize:13, fontWeight:700, cursor:"pointer", transition:"all 0.2s" }}>
                  ☕ 점수 높은 사람
                </button>
                <button onClick={() => setPenaltyMode("low")} style={{ flex:1, background: penaltyMode==="low" ? "linear-gradient(135deg,#8b5cf6,#6366f1)" : "rgba(255,255,255,0.07)", border:"none", borderRadius:12, padding:"12px", color: penaltyMode==="low"?"#fff":"#aaa", fontSize:13, fontWeight:700, cursor:"pointer", transition:"all 0.2s" }}>
                  ☕ 점수 낮은 사람
                </button>
              </div>
            </div>
            <button onClick={startGame} style={{ background:"linear-gradient(135deg,#eab308,#ca8a04)", border:"none", borderRadius:18, padding:"15px 48px", color:"#fff", fontSize:17, fontWeight:800, cursor:"pointer", boxShadow:"0 6px 24px rgba(234,179,8,0.5)" }}>
              🎲 던지기 시작!
            </button>
          </>
        )}
        ...
        {/* Rematch banner */}
        {phase === "rematch" && (
          <div style={{ background:"rgba(239,68,68,0.15)", border:"1px solid rgba(239,68,68,0.4)", borderRadius:14, padding:"10px 16px", textAlign:"center", width:"100%" }}>
            <div style={{ fontSize:14, fontWeight:800, color:"#f87171" }}>🔥 동점 재경기!</div>
            <div style={{ fontSize:12, color:"#888", marginTop:2 }}>{roundParticipants.join(", ")} 재대결</div>
          </div>
        )}

        {(phase === "playing" || phase === "rematch") && (
          <>
            <div style={{ display:"flex", gap:5, justifyContent:"center", flexWrap:"wrap" }}>
              {roundParticipants.map((name, i) => (
                <div key={i} style={{ background: i<roundResults.length ? `${COLORS[allParticipants.indexOf(name)%COLORS.length]}22` : i===currentIdx ? `${COLORS[allParticipants.indexOf(name)%COLORS.length]}44` : "rgba(255,255,255,0.05)", border: i===currentIdx ? `1.5px solid ${COLORS[allParticipants.indexOf(name)%COLORS.length]}` : "1px solid rgba(255,255,255,0.08)", borderRadius:9, padding:"4px 10px", fontSize:12, fontWeight:700, color: i===currentIdx ? COLORS[allParticipants.indexOf(name)%COLORS.length] : "#555" }}>
                  {name}{i<roundResults.length ? ` (${roundResults[i].total})` : ""}
                </div>
              ))}
            </div>
            <div style={{ fontSize:13, fontWeight:700, color:currentColor, textAlign:"center" }}>
              {rolling ? "🎲 구르는 중..." : `${roundParticipants[currentIdx]} — 드래그해서 던지세요!`}
            </div>
            <div ref={areaRef}
              onMouseDown={e => startDrag(e.clientX, e.clientY)} onMouseUp={e => endDrag(e.clientX, e.clientY)}
              ...
              {!rolling && <div style={{ position:"absolute", top:"50%", left:"50%", transform:"translate(-50%,-50%)", fontSize:12, color:"rgba(255,255,255,0.12)", pointerEvents:"none", textAlign:"center" }}>여기서 드래그</div>}
              <div style={{ position:"absolute", left:dicePos.d1.x-26, top:dicePos.d1.y-26, pointerEvents:"none" }}>
                <Dice3D value={diceVals[0]} rolling={rolling} color={currentColor} size={52} angle={dicePos.d1.angle} />
              </div>
              <div style={{ position:"absolute", left:dicePos.d2.x-26, top:dicePos.d2.y-26, pointerEvents:"none" }}>
        ...
        {phase === "done" && (
          <div style={{ textAlign:"center", background:"rgba(249,115,22,0.15)", border:"1px solid rgba(249,115,22,0.4)", borderRadius:18, padding:"16px", width:"100%", animation:"popIn 0.4s ease" }}>
            ...
            <div style={{ fontSize:22, fontWeight:900, color:"#fb923c" }}>{roundResults.find(r=>r.total===penaltyScore)?.name}</div>
            <div style={{ fontSize:13, color:"#aaa", marginTop:4 }}>
              {penaltyScore}점 — 커피 한 잔 쏘세요!
            </div>
          </div>
        )}
