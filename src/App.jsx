...
import ChovyGame from "./games/ChovyGame";
import HorseGame from "./games/HorseGame";
...
const BETS = [
  ...
  { id: 6, emoji: "🐎", title: "경마 대결", description: "각자 말을 골라 경마를 즐겨요! 꼴찌 말 주인이 커피를 쏩니다.", recommendedCount: "2~8명", winnerCount: "꼴찌 1명", category: "뽑기", color: "#10b981", tag: "신규", game: "horse", needParticipants: true },
  { id: 7, emoji: "🏃", title: "쵸비 피하기", description: "각자 한 번씩 플레이! 쵸비를 가장 오래 피한 사람이 살아남아요.", recommendedCount: "누구든", winnerCount: "최고기록", category: "기록", color: "#06b6d4", tag: "신규", game: "chovy", needParticipants: false },
];
...
export default function App() {
  ...
  const pickRandom = () => {
    const pool = filteredBets.length ? filteredBets : BETS;
  ...
  const handleSelectBet = (bet) => {
    ...
    setParticipants([]);
    setSearchQuery("");
    if (bet.needParticipants) {
      setScreen("select-participants");
    } else {
      setScreen("game");
    }
  };
        ...
        {screen === "home" && (
          <>
            <div style={{ padding: "28px 20px 0", display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
              ...
              <button onClick={pickRandom} style={{ background: "linear-gradient(135deg,#f97316,#ef4444)", border: "none", borderRadius: 14, padding: "10px 15px", color: "#fff", fontSize: 13, fontWeight: 700, cursor: "pointer", display: "flex", alignItems: "center", gap: 6, boxShadow: "0 4px 18px rgba(249,115,22,0.45)", whiteSpace: "nowrap" }}
          ...
          <div style={{ padding: "24px 20px 0" }}>
            ...
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
              ...
              <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
                ...
                <div style={{ background: participants.length > 0 ? "linear-gradient(135deg,#8b5cf6,#6366f1)" : "rgba(255,255,255,0.07)", borderRadius: 20, padding: "4px 13px", fontSize: 13, fontWeight: 700 }}>{participants.length}명</div>
              </div>
            </div>

            <div style={{ minHeight: 44, display: "flex", flexWrap: "wrap", gap: 7, marginBottom: 14, alignContent: "flex-start", padding: "2px 0" }}>
              {participants.map(name => (
                <div key={name} style={{ background: "linear-gradient(135deg,#8b5cf6,#6366f1)", borderRadius: 20, padding: "6px 10px 6px 13px", fontSize: 13, fontWeight: 600, display: "flex", alignItems: "center", gap: 6 }}>
                  {name}
            ...
            <div style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 20, padding: 16, marginBottom: 80 }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
                <div style={{ fontSize: 13, color: "#777", fontWeight: 600 }}>플랩풋볼 팀원 <span style={{ color: "#555" }}>({ALL_MEMBERS.length}명)</span></div>
              </div>
              <input value={searchQuery} onChange={e => setSearchQuery(e.target.value)} placeholder="🔍 이름 검색..." style={{ width: "100%", background: "rgba(255,255,255,0.06)", border: "1px solid rgba(255,255,255,0.11)", borderRadius: 12, padding: "9px 13px", color: "#f0f0f0", fontSize: 13, outline: "none", marginBottom: 12, boxSizing: "border-box" }} />
              <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
                {filteredMembers.map(name => (
                  <div key={name} onClick={() => toggleParticipant(name)}
                    style={{ background: participants.includes(name) ? "linear-gradient(135deg,#8b5cf6,#6366f1)" : "rgba(255,255,255,0.08)", border: participants.includes(name) ? "none" : "1px solid rgba(255,255,255,0.12)", borderRadius: 12, width: 64, height: 40, fontSize: 13, fontWeight: 600, cursor: "pointer", userSelect: "none", color: participants.includes(name) ? "#fff" : "#ccc", display: "flex", alignItems: "center", justifyContent: "center" }}>
            ...
            {reportSent ? (
              ...
              <>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
                  ...
                  <button onClick={submitReport} disabled={!reportForm.title.trim()} style={{ background: reportForm.title.trim() ? "linear-gradient(135deg,#8b5cf6,#6366f1)" : "rgba(255,255,255,0.06)", border: "none", borderRadius: 15, padding: "14px", color: reportForm.title.trim() ? "#fff" : "#444", fontSize: 15, fontWeight: 800, cursor: reportForm.title.trim() ? "pointer" : "not-allowed" }}>제보 보내기 →</button>
                </div>
              </>
            )}
          </div>
